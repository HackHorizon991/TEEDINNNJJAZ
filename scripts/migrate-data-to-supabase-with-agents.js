  ...
  for (const agent of agents) {
    try {
      ...
      if (userCheckError && userCheckError.code !== 'PGRST116') {
        console.error(`เกิดข้อผิดพลาดในการตรวจสอบผู้ใช้ ${agent.id}:`, userCheckError);
      }
      ...
      if (!existingUser) {
        ...
        if (createUserError) {
          console.error(`เกิดข้อผิดพลาดในการสร้างผู้ใช้ ${agent.id}:`, createUserError);
      ...
      if (agentCheckError && agentCheckError.code !== 'PGRST116') {
        console.error(`เกิดข้อผิดพลาดในการตรวจสอบเอเจนต์ ${agent.id}:`, agentCheckError);
      }
      ...
      if (!existingAgent) {
        ...
        if (createAgentError) {
          console.error(`เกิดข้อผิดพลาดในการสร้างเอเจนต์ ${agent.id}:`, createAgentError);
        } else {
